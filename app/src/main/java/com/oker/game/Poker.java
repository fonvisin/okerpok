...
public class Poker {
	...
	byte mState = CardRules.NOTHING;
	GraffiasMaker mGR;
	public Poker(GraffiasMaker gr)
	{
		mGR = gr;
	}
	byte[] sort(byte[] x)
...
//		boolean  straight = false;
		...
		/*if ( cardvals[4] - cardvals[1] == -3  &&              //  Consistent with 
			...
			mState = CardRules.ROYALIST;	//info.value = "Royal flush!"; won = bet * 2500;
		}*/
		if ( cardvals[0] - cardvals[3] == 3  &&              //  Consistent with 
				cardvals[0] - cardvals[4] == 12 &&              //  A, T, J, Q, K...
				flush ) {        //  If we also have a flush, then its a royal flush
			mState = CardRules.ROYALIST;	won++;//info.value = "Royal flush!"; won = bet * 2500;
		}else if ( cardvals[0] - cardvals[4] == 4 && flush ) {
			mState = CardRules.STRAIGHTENS;won++;	//info.value = "Straight flush!"; won = bet * 250;
		}
		...
		if ( won == 0 ) {           // Don't check further if we've already won
			if ( fours > 0 ) {
				mState = CardRules.FORESKIN;	//info.value = "Four of a kind!"; won = bet * 100;
				for(int m = 0;m<5;m++)
					if(_cardvals[m]%13!=mGR.mCard[pn].mBig)
						_cardvals[m] =-1;
			}
			else if ( threes == 1 && pairs == 1 ) {
				mState = CardRules.FALLOUT;	//info.value = "Full house!"; won = bet * 50;
			}
			else if ( flush ) {
				mState = CardRules.FLUSH;	//info.value = "A flush!"; won = bet * 20;
			}
					...
					mState = CardRules.STRAIGHT;	//info.value = "A straight!"; won = bet * 15;
				
			}
			else if ( threes > 0 ) {
				mState = CardRules.THREEFOLD;	//info.value = "Three of a kind!"; won = bet * 4;
				for(int m = 0;m<5;m++)
					if(_cardvals[m]%13!=mGR.mCard[pn].mBig)
						_cardvals[m] =-1;
			}
			else if ( pairs == 2 ) {
				mState = CardRules.TAPIR;	//info.value = "Two pair!"; won = bet * 3;
				for(int m = 0;m<5;m++)
				{
					int n = 0;
					for(;n<5;n++)
			...
			/*else if ( matched[0]  == 2 ||
				...
				mState = CardRules.PAIR;	//info.value = "Jacks or better!"; won = bet * 2;
				for(int m = 0;m<5;m++)
					if(_cardvals[m]%13!=mGR.mCard[pn].mBig)
						_cardvals[m] =-1;
			}
			else {
				mState = CardRules.NOTHING;	//info.value = "No win! Bad luck!";
			}
		}
		for(int m = 0;m<5;m++)
		{
...
//		System.out.println("  ~~~~~~~~~~~~~~~~~~~~~~~~~~~   "+info.value+"  ~~~~~~~~~~~~~~~~~~~~~~~~~~~");
	...
	byte CheckState(byte pn)
	{
		...
		return CardRules.NOTHING;
	}
	byte isPaire(int playerNo)
    {
		if((mGR.cardShuffle[playerNo*2]%13)==(mGR.cardShuffle[playerNo*2+1]%13))
		{
			mGR.mCard[playerNo].mState = CardRules.PAIR;
		}
		else
		{
			mGR.mCard[playerNo].mState = CardRules.NOTHING;
		}
	...
	void CheckGameWin()
	{
		...
		for(int i =0;i<5;i++)
		{
			...
			{
				if(mGR.mCard[i].mState > CardRules.FOLD)
					mGR.mCard[i].mState = CardRules.SHOWCASE;
			}
			else
			{
				counter++;
		...
		byte temp = -1;
		//if((st[0]==CardRules.PAIR || st[0]==CardRules.THREEFOLD||st[0]==CardRules.TAPIR||st[0]==CardRules.FORESKIN||st[0]==CardRules.FALLOUT) && counter >1)
		if(counter >1)
		{
			counter =0;
			for(int i =0;i<5;i++)
		...
		if((st[0]== CardRules.NOTHING) && counter >1)
		{
			temp =-1;
			counter =0;
			for(int i =0;i<5;i++)
...
//					System.out.println(i+"  ~~~------------~~~~~~ "+correct[m]);
		...
		CardRules.GameScreen = CardRules.GAMEWIN;
	}
