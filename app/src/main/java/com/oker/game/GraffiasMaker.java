...
public class GraffiasMaker implements Renderer
{
	final Group root;
	Context mContext;
	...
	SimplePlane[] mTex_Plyar;
	SimplePlane[] mTex_Condition;
	...
	public GraffiasMaker(Context context)
	{
		mContext = context;
		mStartActivity = (StartActivity)mContext;
		root = new Group(this);
		init();
	}
...
//			mTex_RateUs			= add("button/rateus.png");
			...
			SimplePlane[] mTex_CardSelect = new SimplePlane[2];
	...
	void gameReset()
	{
		...
		for(int i =0;i<mCard.length;i++)
        {
			...
			if(mCard[i].mTotalCoin>0)
				...
				mCard[i].mState = CardRules.GAMEST;k++;}
        }
		if(mCard[CardRules.ID].mState <= CardRules.FOLD)
		{
		...
		{
			CardRules.GameScreen = CardRules.GAMECONG;
			NewCong ++;
			if(NewCong* CardRules.MAXAMOUNT* CardRules.MAXAMOUNT*mCard.length > mHighScore)
			{
				mHighScore = NewCong* CardRules.MAXAMOUNT*mCard.length;
			}
		}
		else
        ...
        {
			if(mCard[m%mCard.length].mState > CardRules.FOLD)
			{
...
//				+" ~6~ "+mDealer.mShowCard+" ~7~ "+mDealer.mStartActivity+" ~8~ "+mDealer.mTableBat+" ~9~ "+mDealer);
		...
		AniCounter = 0;
		if(CardRules.GameScreen == CardRules.GAMEPLAY && !isReset)
		if (ads % 3 == 0 || isReset){
	...
	void Reset()
	{
		for(int i =0;i<mCard.length;i++)
        {
			mCard[i].set(CardRules.MAXAMOUNT);
        }
		mDealer.mTableBat = 1;
		isReset = true;
		gameReset();
	...
	void ChardReset()
    {
        ...
        for(byte k =0;k<cardShuffle.length;k++){
            for(byte j =0;j<cardShuffle.length;j++)
            {
               ...
               /* if(j%5==0)
                {
                    a = (byte)((Math.abs(CardRules.mRand.nextDouble()*10000000))%52);
                    b = (byte)((Math.abs(CardRules.mRand.nextFloat()*10000000))%52);
                }
                else if(j%5==1)
                {
                    a = (byte)((Math.abs(CardRules.mRand.nextFloat()*10000000))%52);
                    b = (byte)((Math.abs(CardRules.mRand.nextLong()*10000000))%52);
                }
                else if(j%5==2)
                {
                    a = (byte)((Math.abs(CardRules.mRand.nextLong()*10000000))%52);
                    b = (byte)((Math.abs(CardRules.mRand.nextInt()*10000000))%52);
                }
                else if(j%5==3)
                {
                    a = (byte)((Math.abs(CardRules.mRand.nextDouble()*10000000))%52);
                ...
                {
                    a = (byte)((Math.abs(CardRules.mRand.nextFloat()*10000000))%52);
                    b = (byte)((Math.abs(CardRules.mRand.nextInt(52)*10000000))%52);
                }*/
                a = (byte)Math.abs(CardRules.mRand.nextInt(52));
                b = (byte)Math.abs(CardRules.mRand.nextInt(52));
                temp = cardShuffle[a];
...
//			System.out.println("cardShuffle["+k+"] = "+cardShuffle[k]+";");
	...
	SimplePlane add (String ID)
	{
		...
		try
		{
			SP = new SimplePlane((b.getWidth()/ CardRules.mMaxX),(b.getHeight()/ CardRules.mMaxY));
